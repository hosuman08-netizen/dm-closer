...
(function(){
  ...
  function saveHist(h){try{localStorage.setItem('dm_hist',JSON.stringify(h.slice(0,15)));}catch(e){}}
  function bumpStreak(){
    try{
  ...
  function render(msg){
    ...
    root.innerHTML='<div class="card"><div class="sub">톤 4종 · 오늘 '+todayN()+'초안 · 🔥'+sc+'일 · 이력 '+h.length+'</div>'
      ...
      +'</select><button id="go">DM 초안</button>'
    ...
    if(h.length){
      document.getElementById('hist').innerHTML=h.slice(0,6).map(function(x,i){
        return '<div data-i="'+i+'" style="padding:6px 0;border-bottom:1px solid #2a2438;cursor:pointer">['+tones[x.tone]+'] '+(x.prod||'').slice(0,28)+' · 탭=재로드</div>';
      }).join('');
